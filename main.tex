...
For presentation, we use \red{red} terms to denote public terms (e.g., \red{$A$}), 
\blue{blue} terms to denote fresh terms (e.g., \blue{$N_a$}); 
everything else has sort \textsc{Msg}.
...
and $\mathsf{s\_enc}(v_1, \blue{v_2}) \in \mathcal{T}(\Sigma, D)$. 
...
\rowcolor{grouponeDark}
\text{identity of }\oplus & = \red{\mathsf{keystream}(k, IV)}& \\[.8em]
...
&\triangleq \{ \mathsf{In}(c),
      ...
      \mathsf{StA1}(\red{A}, \red{B}, \blue{n_A}) \} \xrightarrow{\{\}}
\\
& 
   \hspace{14pt}\{ \mathsf{StA2}\!\left(
        ...
        \red{B},
        \blue{n_A},
        \pi_2(\mathsf{a\_dec}(c, \blue{sk_A}))
      \right),
\\
& 
      \hspace{28pt}\mathsf{Out}\!\left(
...
&\triangleq \{ \mathsf{In}(c),
      !\mathsf{Ltk}(\red{B}, \blue{sk_B}),
      \mathsf{StB1}(\red{B}, \red{A}, \blue{n_A}, \blue{n_B}) \} \xrightarrow{\{\}}
\\
& 
   \hspace{14pt}\{ \mathsf{Commit}(\red{B}, \red{A}, \blue{n_A}, \blue{n_B}) \}.
\end{align*}
...
The first rule, \textsf{Register}, represents the registration of an asymmetric key pair to identity $\red{A}$. Note that $A^\textsc{pub}$, along with the other identifiers in the rule descriptions, are \textit{variables} and can be instantiated with different values in different rule instances. Hence, the protocol description can capture an unbounded number of sessions and principals. The \textsf{Ltk} (short for ``long-term key") stores $\red{A}$'s (fresh) secret key $\blue{sk}$, while $\textsf{Pk}$ stores the public key derived from $\blue{sk}$. Finally, the identity/public key mapping are broadcast along the network channel, as they are public information that should be known by the adversary for realistic modeling.
