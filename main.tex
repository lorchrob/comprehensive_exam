...
\begin{itemize}
    ...
    then there exists a \emph{unique} event in which $B$ sent data item $d$, apparently to $A$. 
\end{itemize}
...
is to substitute $\langle N_A, N_B, B \rangle$ for $\langle N_A, N_B \rangle$ in step $2$ 
...
to be useful; we outline the first few steps representing key generation. At each step, we outline the rule used and the updates to state (a multiset of ground facts).
...
$P \rightarrow^* C[\bar{c}\langle M \rangle]$  for some context $C$ such that $a$ is not bound in $C$. 
...
The details of $\rightsquigarrow$ will not be discussed in this report; however, we outline a few technical challenges.
...
it returns a \emph{complete set of unifiers} \cite{fvp_unification}.
...
Consider the following attack trace with intruder $I$, presented in Figure \ref{fig:type-confusion-attack}. In this trace, the capital letters 
...
This also explains why using the triple $\langle N_a, N_b, B \rangle$ is not vulnerable to the same attack.
