...
even means in the context of security protocols is itself a nontrivial question, even before attempting a proof. 
The general difficulty of this problem has resulted in flawed ``proofs" of correctness, leading to real-world security impacts. 
...
and have some automated tool either prove the property correct or return a concrete attack trace,
...
that requires expertise in \emph{two} separate formalisms, potentially hinting toward an area for future improvement.
...
Additionally, we will discuss implemented tools, but with a special focus on the two that dominate modern 
...
along with some of the major challenges. Next, we discuss metatheoretic results in the symbolic model 
...
Historically, it largely centered around the study of \textit{ciphers}, 
...
Each \textit{plaintext} $p \in \mathcal{M}$ intuitively represents a message to be shared, 
each \emph{ciphertext} $c \in \mathcal{C}$ represents an obscured plaintext, 
and each \emph{key} $k \in \mathcal{K}$ can be used to help generate ciphertexts from plaintexts. 
A \emph{symmetric-key encryption scheme} is a triple $(\mathsf{Gen}, \mathsf{Enc}, \mathsf{Dec})$ 
comprised of three functions 
$\textsf{Gen} : \{1\}^+ \rightarrow \mathcal{K}$, $\textsf{Enc} : \mathcal{M} \times \mathcal{K} \rightarrow \mathcal{C}$, 
and $\textsf{Dec} : \mathcal{C} \times \mathcal{K} \rightarrow \mathcal{M}$, 
where $\textsf{Gen}(1^\lambda)$ probabilistically samples a key of length $\lambda$ from 
...
it reveals no information about the value of its plaintext $m$. 
...
If $|k| = \lambda$ for all $k \in \mathcal{K}$ and $M = \{0,1\}^n$ for some $n$, 
...
Additionally, $A$ can \emph{digitally sign} of message $m$ with $A$'s private key, 
...
Notation $f \circ g(x)$ denotes function composition of the form $g(f(x))$ for input $x$ 
...
and we use postfix notation to denote the application of a substitution (e.g., $t\{x \mapsto M\}$). 
...
and we use $f[x \mapsto y]$ to denote the function that agrees with $f$, 
save for mapping $x$ to $y$. Further preliminaries for more specific notation will be introduced later in the report, 
...
\begin{itemize}
    ...
    \emph{initialization vector} \textit{IV} (essentially, a random value).
...
For goal (1), idealized random encryption has stronger secrecy guarantees compared to deterministic encryption, 
...
that the hash of the plaintext (after decrypting) matches the hash sent by $A$. 
...
While this does not directly leak the plaintext, it is still a form of information leakage 
...
security properties can be partitioned into \emph{trace properties} and \emph{hyperproperties} \cite{hyperproperties}. 
\emph{Trace properties} are defined over single traces, while \emph{hyperproperties} are defined in terms of sets of traces.
...
\begin{itemize}
    ...
    we do not discuss it further in this document.)
    ...
    whenever principal $A$ accepts data item $d$, apparently from $B$, 
    then there exists a \emph{unique} prior trace of $B$ in which $B$ sent data item $d$, apparently to $A$. 
\end{itemize}
...
subsequent time that $A$ thinks that $A$ has received a message $m$ from $B$, $B$ must have sent it again, 
...
where server responses to adversary-mauled ciphertexts reveal information about the underlying plaintext 
(without directly leaking it). In these attacks, 
so much information is revealed that the adversary is eventually able to reconstruct the \emph{entire} plaintext.
...
More precisely, \emph{fr} and \emph{pub} are incomparable and both subsorts of \emph{msg}. 
...
and $\mathsf{s\_enc}(v_1^{\textsc{Msg}}, v_2^\textsc{Fr}) \in \mathcal{T}(\Sigma, D)$. 
...
For brevity, we may omit the $\textsc{Msg}$ superscript for variables with sort \emph{msg}. 
...
of equations $E$, where each equation is comprised of two terms in $\mathcal{T}(\Sigma, V)$. 
...
using the same key that it was encrypted with yields the underlying plaintext. 
...
For example, with this $E$, we get $\mathsf{enc}(\mathsf{dec}(\mathsf{enc}(d_1, k_1), k_1), k_2) =_E \mathsf{enc}(d_1, k_2)$.



\textbf{Attacker capabilities.} 
Given a set of public terms $P$ and an insecure network $N$ (purposefully left informal for now), 
we can inductively define the set of terms $\mathcal{K}$ in the knowledge base of a symbolic model attacker, 
...
\subsection{Weaknesses of the symbolic model} Note that the only way for a Dolev-Yao adversary 
...
\textit{only way} to recover the plaintext from a ciphertext is to have knowledge of the key. 
...
The derivation is outlined in Figure \ref{fig:d}. In the first computation, 
notice that $\textsf{sr\_enc}(m, k, \textit{IV}) = m \oplus \textsf{keystream}(k, \textit{IV})$ 
(by definition of \textsf{sr\_enc}), which implies 
$\textsf{keystream}(k, \textit{IV}) = \textsf{sr\_enc}(m, k, \textit{IV}) \oplus m$. 
Because $\textsf{sr\_enc}(m, k, \textit{IV})$ and $m$ are both sent along the channel, 
the adversary can compute $\textsf{keystream}(k, \textit{IV})$. In the second step, 
notice that $\textsf{sr\_enc}(s, k, \textit{IV}) = s \oplus \textsf{keystream}(k, \textit{IV})$ 
(by definition of \textsf{sr\_enc}), which implies 
$s = \textsf{sr\_enc}(s, k, \textit{IV}) \oplus \textsf{keystream}(k, \textit{IV})$. 
Because $\textit{IV}$ was erroneously reused, the attacker still knows the keystream and 
(along with knowledge of $\textsf{sr\_enc}(s, k, \textit{IV})$) can directly compute $s$. 
...
as the attacker needs knowledge of $k$ to destruct $\textsf{sr\_enc}(s, k, \textit{IV})$ 
...
A famous example is the attack found on the Needham-Schroeder public key authentication protocol \cite{ns_attack}, 
...
\end{itemize}
\end{algorithm}
Algorithm \ref{alg:unification-mod-e} is sound and complete \cite{fvp_unification};
it returns a \emph{complete set of unifiers} which, intuitively speaking, 
is a set of unifiers such that any unifier is less general than at least one in the set \cite{fvp_unification}.
...
However, the translation rules can be found in Figure \ref{fig:horn-translation} in the appendix. 
...
modulo an equational theory \cite{folding_variant_narrowing}).
...
Why can we not use $E_{\oplus}$ in Tamarin? None of the semantics presented in Section \ref{sec:msr-approaches}, 
...
\textbf{Rewriting modulo E.} To take advantage of this partition of $E_\oplus$, 
...
The notion of $E$-convergence generalizes convergence in the expected way, and
...
by lifting every normalized term $t\!\downarrow_{\mathcal{R}}$ to terms normalized modulo $E$
...
Then, the procedure for unification modulo $E$ can proceed similarly to Algorithm \ref{alg:unification-mod-e}, with two updates.
First, instead of computing variants under a purely convergent
theory, we compute a complete set of \emph{AC}-variants 
of $t_1$ and $t_2$ from the equation $t_1 = t_2$ using a technique called folding variant narrowing
...
For each such $\mu$, we return 
$\tau \circ \mu$.
Crucially, Algorithm \ref{alg:unification-mod-e} still terminates, 
as unification modulo associativity and commutativity axioms is decidable 
with existing algorithms from the literature \cite{unification_modulo_ac}.
Again, the lifted (modulo \emph{AC}) version of this algorithm is sound and complete \cite{tamarin}, 
assuming confluence, coherence, and the finite property modulo \emph{AC}.
...
The result is formally proven in \cite{tagging}. 
...
\begin{itemize}
    ...
    \item[\textsf{RQ1.}] 
    \emph{How to reason about cryptographic protocols with heavy non-cryptographic aspects in a single formalism, 
    ...
    \item[\textsf{RQ2.}] 
    \emph{How to reason about protocols with unbounded loops and recursive datatypes, 
    ...
    \item[\textsf{RQ3.}] 
    \emph{How to reason about cryptographic protocols compositionally?} 
    ...
    in general, 
    and the combined cost of inferring decompositions and compositional reasoning may not even be more efficient 
    ...
    \item[\textsf{RQ4.}] \emph{How to reason about the composition of cryptographic protocols?} 
    Consider two protocols $P_1$ and $P_2$, and assume you prove that $P_1$ both satisfies security property 
    $\varphi$ (i.e., $P_1 \models \varphi$). 
    What can you say about the asynchronous parallel composition of $P_1$ and $P_2$? 
...
\begin{figure}
  ...
  \begin{align*}
    ...
    \llbracket \textbf{new }n; P \rrbracket \rho H &= \llbracket P \rrbracket \rho[n \mapsto n[p'_1, \dots, p'_m]] H \\
    \llbracket x(M); P \rrbracket \rho H &= \llbracket P \rrbracket \rho[x \mapsto x] (H \land message(\rho(M), x)) \\
    \llbracket \bar{N}\langle M\rangle; P \rrbracket \rho H &= \llbracket P \rrbracket \rho H \cup \{  H \Rightarrow message(\rho(M), \rho(N)) \}\\
    ...
    \cup \{ \llbracket P \rrbracket ((\sigma \rho)[x \mapsto \sigma'p']) (\sigma H) \mid \\
    & \qquad g(p'_1, \dots, p'_n) \rightarrow p'\text{ and }(\sigma, \sigma')\text{ is the most general} \\
    & \qquad \text{pair of substitutions such that }\\
    & \qquad \sigma \rho(M_1) = \sigma'p'_1, \dots, \sigma \rho(M_N) = \sigma'p'_n \} \cup \llbracket Q \rrbracket \rho H
\end{align*} 
\caption{ProVerif's translation from ProVerif's variant of the applied $\pi$-calculus to Horn clauses}\label{fig:horn-translation}
\end{figure}
