...
Additionally, there is a middle ground called the \textit{computationally complete symbolic attacker (CCSA) model}.
...
For example, we could have $(\mathsf{enc}, \mathit{msg} \times \mathit{msg} \rightarrow \mathit{msg}) \in \Sigma_c$ 
and $\mathsf{enc}(\red{v_1}, \blue{v_2}) \in \mathcal{T}(\Sigma, D)$. 
...
In the most basic setting, we have $E = \{ \mathsf{dec}(\mathsf{enc}(m, k), k) = m \}$ 
with 
$\Sigma = \{ \mathsf{enc} :: \mathit{msg} \times \mathit{msg} \rightarrow \mathit{msg}, 
\mathsf{dec} :: \mathit{msg} \times \mathit{msg} \rightarrow \mathit{msg} \}$. 
Intuitively, the equation $\mathsf{dec}(\mathsf{enc}(m, k), k) = m$ states that decrypting a ciphertext 
...
\textsc{Pub} states that if a term $t$ is public, then it is in the adversary's knowledge; 
...
As an example, assume $\textsf{enc}(m, k)$ was sent on $N$ and that $k \in \mathcal{K}$. 
From this, we can apply \textsc{Net} to $\textsf{enc}(m, k)$ and include it in $\mathcal{K}$. 
Then, because $k$ and $\textsf{enc}(m, k)$ are both in $\mathcal{K}$, 
we can add $\textsf{dec}(\textsf{enc}(m, k), k)$ to $\mathcal{K}$ (using \textsc{Fun}). 
Finally, with \textsc{Eq}, since $\textsf{dec}(\textsf{enc}(m, k), k) =_E m$, 
...
A protocol satisfies weak secrecy if the adversary never learns secret terms
...
\textsf{sr\_dec} :: \mathit{msg} \times \mathit{msg} \rightarrow \mathit{msg} \} \text{ and} \\ 
...
as the implementation details of $\textsf{sr\_enc}(.,.,.)$ are not specified, and
...
and hence $\xrightarrow{a}_R \;\subseteq \mathcal{S} \times \mathcal{S}$. 
  \begin{mathpar}
    \inferrule*
      { l \xrightarrow{a} r \in \mathsf{ginsts}_E(R) \quad l \subseteq^\# S }
...
\end{gather*}
The protocol is encoded as a set of rewrite rules $R = \{ \mathsf{Register}, \mathsf{Reveal}, A_1, B_1, A_2, B_2 \}$, 
...
Finally, the identity/public key mappings are broadcast along the network channel, 
...
\subsection{Applied $\pi$-calculus-based approaches}\label{sec:apc-approaches}
...
$C[\square] \triangleq (\square \mid c(M). \textsf{if }M=\textsf{ok} \textsf{ then } \bar d\langle \textsf{ok} \rangle \textsf{ else } 0.)$ 
In general, the adversary can always perform \emph{tests} in the guard of \textsf{if} expressions 
...
We denote the multi-step labeled relation (induced from $\xrightarrow{\alpha}$) as $\xrightarrow{\alpha_1,\dots,\alpha_n}$. Given $\mathcal{K}\xrightarrow{\alpha_1,\dots,\alpha_n}\mathcal{K}'$, we can extract 
...
Consider the unification problem for terms $y$ and  $\textsf{dec}(x,k)$
...
\begin{itemize}
    ...
    \item[(1)] The variables of $y$ and $\textsf{dec}(x,k)$ are already disjoint.
...
\begin{array}{cll}
   ...
   (tr, \theta) \models_E  & i^\textsc{Tmp} = j^\textsc{Tmp} & \text{if }\theta(i^\textsc{Tmp}) = \theta(j^\textsc{Tmp}) \\
   (tr, \theta) \models_E  & t_1 = t_2 &\text{if } \theta(t_1) =_E \theta(t_2) \\
...
\begin{itemize}
    ...
    This closely aligns with the natural language definition of non-injective agreement: if $A$ accepted term $t$ (apparently) from $B$, then $B$ sent term $t$ (apparently) to $A$ at some earlier time.
...
\end{gather*}
The formalization of injective agreement is the same as non-injective agreement, 
save for the extra conjunct on the second line. 
It states that if $A$ accepted $t$ from $B$, then $B$ sent $t$ to $A$, 
\emph{and also} there was not another distinct accept event for the same term $t$. 
At first glance, the property may seem too strong---for example, 
what if $B$ sends $t$ twice, and $A$ accepts $t$ twice? 
It falsifies this property, but should not violate the textbook definition 
of injective agreement (from Section \ref{sec:security-properties}). 
Although this trace technically does not violate the textbook definition, 
one can easily construct another trace from it that \emph{does} violate it---simply replace the second honest send with an adversarial replay. Hence, each term sent by an honest participant must include some notion of freshness (e.g., via a nonce or counter) if a protocol hopes to satisfy injective agreement.
\end{itemize}
...
In this setting, we consider the \textit{predicate signature} 
$ \Sigma_p = \{ (\textsf{Attacker}, \emph{msg}), (\textsf{Message}, \mathit{msg} \times \mathit{msg}) \}$ 
and predicate arguments from $\mathcal{T}(\Sigma_c, V)$.
...
Consider the signature $\Sigma_c = \{ f :: \mathit{msg}, c_1 :: \mathit{msg}, c_2 :: \mathit{msg} \}$ with predicate symbols $p_1$ and $p_2$, and consider the Horn clause set $\{ p_1(f(c_1)), p_1(f(x)) \Rightarrow p_2(f(x)) \}$.  Then, an example Herbrand model is $\{ p_1(f(c_1)), p_2(f(c_1)), p_1(f(c_2)) \}$,  and the least Herbrand model is $\{ p_1(f(c_1)), p_2(f(c_1)) \}$. 
...
between consequents and antecedents (see injective agreement below), as well as temporal variables and constraints.


\textbf{Standard security properties in ProVerif.}
\begin{itemize}
    \item \emph{Weak secrecy}: $$\mathsf{Attacker}(t)$$
    We check for the derivability of term $t$ by the adversary. If so, weak secrecy is violated.
    ...
    The property is encoded as a correspondence query between a term $t$ being accepted by $A$ from $B$ and being sent from $B$ to $A$.
    \item \emph{Injective agreement}: 
    $$\forall A. \forall B. \forall t.\; \textsf{inj-event}(\mathsf{Accept}(A, B, t)) \Rightarrow \textsf{inj-event}(\textsf{Sent}(B, A, t))$$
    The encoding is a correspondence analogous to non-injective agreement, but we simply replace the \textsf{Event} keyword with \textsf{Inj-Event} to denote that the correspondence should be injective.
...
\begin{enumerate}
    ...
    \item[(1)] $A$ sends $\textsf{enc}(\langle N_1, N_3 \rangle, k)$ and $\textsf{enc}(\langle N_2, N_3 \rangle, k)$ to $B$.
    \item[(2)] $B$ sends a message to $A$. If $A$ receives $\textsf{enc}(\langle x, N_3 \rangle, k)$, respond with $x$.
...
\begin{array}{ccl}
   ...
   \mathsf{Message}(\mathsf{enc}(\langle N_1, N_3 \rangle, k), c) & (ii)& \text{protocol rule (1) (part one)}\\
   \mathsf{Message}(\mathsf{enc}(\langle N_2, N_3 \rangle, k), c) & (iii) & \text{protocol rule (1) (part two)}\\ 
   \mathsf{Message}(\mathsf{enc}(\langle x, N_3 \rangle, k), c) \Rightarrow \textsf{Message}(x, c) & (iv) & \text{protocol rule (2)}\\
...
Above, Horn clause $(i)$ denotes that the communication channel is public. Clauses $(ii)$ and $(iii)$ denote that the corresponding messages are sent according to protocol rule $(1)$. Clause $(iv)$ denotes  protocol rule $(2)$---with the given input along $c$, the given output along $c$ is produced. Analogously, clause $(v)$ denotes protocol rule $(3)$. With these clauses, along with the adversary deduction clauses, ProVerif will iteratively derive new clauses in an attempt to conclude $\mathsf{Attacker}(s)$. From the Horn clause adversary deduction rule along with clauses $(ii)$ and $(iii)$, the adversary can learn both $\mathsf{enc}(\langle N_2, N_3 \rangle, k)$ and $\mathsf{enc}(\langle N_1, N_3 \rangle, k)$. Next, ProVerif's algorithm will use clause $(iv)$ \textit{twice}, along with adversary deduction rules, to derive both $\mathsf{Attacker}(N_1)$ and $\mathsf{Attacker}(N_2)$. Importantly, the Horn clause abstraction loses the linearity of $A$'s protocol state---once step $(2)$ is executed, in a precise semantics, $A$  would move on to step $(3)$. In contrast, ProVerif's algorithm will reuse clause $(iv)$, representing a repetition of the protocol step.
...
One can realize this translation with a provably sound transpiler, SAPIC$^+$ \cite{sapic}. 
SAPIC$^+$ stands for \textbf{S}tateful \textbf{A}pplied \textbf{P}i \textbf{C}alculus, 
and it is an extension of the applied $\pi$-calculus to handle state with the same property 
specification language as Tamarin. Let $\mathcal{M}$ be a SAPIC$^+$ protocol model, 
i.e., a process in the applied $\pi$-calculus with state, and let $\varphi$ be a SAPIC$^+$ property. 
Then, the SAPIC$^+$ paper introduces four functions $\llbracket . \rrbracket^{\textsc{TM}}, 
\llbracket . \rrbracket^{\textsc{TP}}, \llbracket . \rrbracket^{\textsc{PM}}, 
\llbracket . \rrbracket^{\textsc{PP}}$, where $\llbracket . \rrbracket^{\textsc{TM}}$ maps 
SAPIC$^+$ models to Tamarin models (i.e., sets of multiset rewriting rules), 
$\llbracket . \rrbracket^{\textsc{TP}}$ maps SAPIC$^+$ properties to Tamarin properties, 
$\llbracket. \rrbracket^{\textsc{PM}}$ maps SAPIC$^+$ models to ProVerif models, and 
$\llbracket . \rrbracket^{\textsc{PP}}$ maps SAPIC$^+$ properties to ProVerif properties. 
In the latter two cases, the translation may fail in the case that the SAPIC$^+$ model 
uses some feature unsupported by ProVerif. 
...
Equation $(3)$ is redundant, but it is included for technical 
...
Applying the identity law $(x \oplus 0 = x)$ on $c \oplus 0$, 
...
Assume $\textsf{Attacker}(\mathsf{enc}(\mathsf{enc}(m, k_2), k_1))$  and $\textsf{Attacker}(k_2)$
are derivable. 
Here, it is not sound to derive $\textsf{Attacker}(\mathsf{enc}(m, k_1))$, 
...
$t_1|_p =_E l\sigma$ and $t_1[r\sigma]_p = t_2$. In other words, $\mathcal{R},E$-rewriting
...
Existing approaches have been successfully applied to analyze protocols with a variety of cryptographic primitives 
...
In this section, we outline efforts to ease reasoning about security protocol verification. While the symbolic model security protocol verification problem is undecidable in general \cite{undecidable1,undecidable2}, one may recover decidability by considering a restricted class of protocols or attacks.  First, we focus on typing protocols, and then we expand the discussion to a broad overview of metatheoretical results.
...
we have $\mathsf{f}(t_1, \dots, t_n)\{\square \mapsto t\}$ normalizes to $t$. 
...
we have that there exists some substitution $\theta$ such that $t \theta = t'\theta$ 
...
by replacing each $c(R)$ with $c(\square)$ and each $\bar c \langle w \rangle$ 
with $\bar c \langle \square \rangle$.
...
Since $\mathsf{tr}$ and $\mathsf{tr}'$ share the same trace skeleton, 
they produce the same barbs, and hence $\mathsf{tr}'$ witnesses 
the same security violation as $\mathsf{tr}$.
...
\begin{itemize}
    ...
    while retaining some  automation.} 
    ...
    Ideally, reasoning could proceed with a single unified model. 
    ...
    without resorting to manually annotated security labels (or more generally, spurious violations), while retaining some automation.} 
    ...
    \item[\textsf{RQ3.}] 
    \textbf{How to reason about cryptographic protocols compositionally while retaining some automation.} 
    ...
    have ``similar" shapes with honest steps and adversaries with similar sets of capabilities. 
    ...
    % This problem has been addressed more thoroughly in the setting of (interactive) \emph{computational} model verification 
    % through the \emph{universal composability} framework \cite{uc}.
    \item[\textsf{RQ4.}] \textbf{How to recover useful interactivity when automated techniques fail to terminate or produce spurious violations.} 
