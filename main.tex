...
Now consider an abstraction $P(s)$ for some process $P$ that is supposed to keep $s$ secret. 
We say that $P$ satisfies the (strong) secrecy of $s$ if for all $s$ and  $s'$, $P(s) \approx P(s')$.
...
In general, the adversary can perform always perform \emph{tests} in the guard of if expressions 
...
to achieve decidable unification and with respect to a broader class of equational theories. 
Tamarin intentionally forgoes some automation because it has stronger support for interactivity. 
In principal, one could take a similar approach with ProVerif, 
...
The first group (in blue) denote \emph{modern CPVs with unbounded analyses}. 
In particular, Tamarin and ProVerif dominate the rest in terms of modern usage \cite{sok}. 
Compared to the rest of the CPVs, Tamarin is surprisingly well-rounded---it imposes no bounds on analysis, 
...
On the other hand, while Maude-NPA's formal foundations are incredibly well-motivated, 
...
The third group (in orange) denote \emph{CPVs based on security by typing}. 
...
The fourth group (in purple) denote \emph{CPVs focused on equivalence properties}. 
...
However, there are many challenges where surprisingly little progress has been made. 
...
To address these issues, the cryptographic protocol verification community may be able to draw inspiration 
from existing approaches in the verification community.
