...
DEEPSEC \cite{deepsec} & applied \newline$\pi$-calculus & sessions & AS, PS & none & trace & subterm- \newline convergent & no/no & no \\

\rowcolor{groupfourLight}
SAT-Equiv \cite{sat_equiv} & applied \newline$\pi$-calculus, SAT & everything & AS, PS & none & trace & encryption/ decryption & no/no & no \\
...
\begin{flushleft}
\vspace{.3em}
Formal guarantees are with respect to the \emph{unbounded} security protocol verification problem. \newline\newline
AS $\mapsto$ attack soundness, AC $\mapsto$ attack completeness, PS $\mapsto$ proof soundness, PC $\mapsto$ proof completeness, LIA $\mapsto$ linear integer arithmetic,
