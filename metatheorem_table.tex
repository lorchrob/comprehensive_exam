...
\small
\centering
\begin{tabular}{|p{2cm}|p{1.8cm}p{2.6cm}p{2.8cm}p{1.72cm}p{2.6cm}|}
\hline
\textbf{Paper} &
\textbf{Formalism} &
\textbf{Restrictions} &
...
Durgin et al. \cite{bounded_undecidable} & multiset rewriting & bound message size and $|\mathcal{D}|$ & EXPTIME-completeness & yes & AS \\

\rowcolor{grouponeDark}
Ramanujam \& Suresh \cite{tagged_bounded_secrecy} & DY & tagged protocols, bounded messages, secrecy & decidability & yes & AC, PC \\
...
\rowcolor{grouptwoLight}
Lowe \cite{small_model} & DY & context-explicit, inferrable identities, no temporary secrets, secrecy & small model theorem (one session with one principal per role) & yes & AS, PS, AC, PC \\
...
Nielson et al. \cite{cubic_time} & DY & secrecy properties & termination \newline (w/ approximation)  & yes & PS, AC \\

\rowcolor{groupfourLight}
Goubault-Larrecq \cite{ai_1} & DY & secrecy properties & termination \newline (w/ approximation) & yes & PS, AC \\

\rowcolor{groupfourDark}
Heather \& Schneider \cite{ai_2} & CSP & auth properties  & termination \newline (w/ approximation) & yes & PS, AC \\
...
AS $\mapsto$ attack soundness, AC $\mapsto$ attack completeness, PS $\mapsto$ proof soundness, PC $\mapsto$ proof completeness
\newline\newline
Formal guarantees describe algorithms proposed by papers with respect to the \emph{unbounded} security protocol verification problem. 
``Fragment decidable'' refers to the decidability of the corresponding \emph{bounded} fragment.
\newline\newline
